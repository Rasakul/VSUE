...
public class TCPWorker implements Worker {
    ...
    public TCPWorker(int ID, Chatserver chatserver, Socket clientSocket, PrintStream userResponseStream) {
        ...
        this.channel = new ObjectChannel(clientSocket);
    }
    ...
    public void run() {
        try {
            ...
            ServerAuthenticator serverAuthenticator = new ServerAuthenticator(this, chatserver, channel, userResponseStream);

            if (serverAuthenticator.isAuthenticated()) {
                while (running) {
